...
KPI_DEFINITIONS = [
    _kpi("Total Inventory Value (Cost)", "Inventory", "Latest inventory snapshot valued at unit cost.", "SUM(inventory_value_at_cost) at MAX(date_key)", "USD", "Inventory Manager", target=15_000_000.0),
    ...
    _kpi("Stock Out Rate", "Inventory", "Share of inventory records classified as out of stock.", "COUNT(Out of Stock) / COUNT(Inventory Records) x 100", "%", "Inventory Manager", target=3.0),
    _kpi("Items Below Reorder Point", "Inventory", "Inventory records requiring replenishment.", "COUNT(needs_reorder = 1)", "Items", "Replenishment Manager", target=50.0),
    ...
    _kpi("Average Transit Days", "Shipment", "Average actual transit duration for delivered shipments.", "AVERAGE(actual_transit_days) WHERE status = Delivered", "Days", "Logistics Manager", target=5.0),
    ...
    _kpi("Cancellation Rate", "Shipment", "Cancelled shipments as a share of all shipments.", "COUNT(Cancelled) / Total Shipments x 100", "%", "Logistics Manager", target=2.0),
    ...
    _kpi("Average Units Per Hour", "Labor", "Processed units per labor hour.", "Total Units Processed / Total Labor Hours", "Units/Hour", "Operations Manager", target=80.0),
    ...
    _kpi("Error Rate", "Labor", "Recorded errors relative to processed units.", "SUM(total_errors) / Total Units Processed x 100", "%", "Quality Manager", target=2.0),
    _kpi("Cost Per Unit Processed", "Labor", "Labor cost required per processed unit.", "Total Labor Cost / Total Units Processed", "USD/Unit", "Finance", target=0.5),
